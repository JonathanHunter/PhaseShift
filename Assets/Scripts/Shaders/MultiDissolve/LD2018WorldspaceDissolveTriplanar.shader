Shader "Custom/LD2018WorldspaceDissolveTriplanar" {
	Properties{
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			else 
			{
				...
				cx = tex2D(_SecondTex, tx) * bf.x;
				cy = tex2D(_SecondTex, ty) * bf.y;
				cz = tex2D(_SecondTex, tz) * bf.z;
				c2 = (cx + cy + cz) * _Color2;
			}
